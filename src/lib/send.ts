...
async function SendEdit(
  ...
  data: WebhookMessageEditOptions
) {
  sendLog(data);
...
function sendLog(data: MessagePayloadOption) {
  const text =
    typeof data === "object" && !data.files
      ? JSON.stringify(data, null, 1)
      : data;
  console.log(
    styleText("bgWhiteBright", styleText("black", "----- Send Log -----"))
  );
  ...
  if (typeof text === "string") {
    ...
    console.log(
      styleText(
        "white",
        `ファイル名: ${JSON.stringify(data.files[0].name, null, 1)}`
      )
    );
...
async function SendCommandError(name: string, error: unknown, text: string, interaction: CommandInteraction) {
  if (error instanceof Error) {
    console.log(name + error.message);
		await SendReply(interaction, GenerateText(`${text}: ${error.message}`));
  } else {
    console.log(name + "未知のエラーが発生しました。");
		console.log(error);
    await interaction.reply("予期せぬエラーが発生しました。");
		await SendReply(interaction, GenerateText(`予期せぬエラーが発生しました。`));
  }
}
